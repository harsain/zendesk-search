...
public class UserServiceTest {
    ...
    public void userServiceFindBy() throws Exception {
        List<Organisation> organisations = mapper.readValue(organisationResource.getInputStream(), new TypeReference<List<Organisation>>(){});

        Mockito.when(organisationService.findById(ArgumentMatchers.any())).thenReturn(organisations.get(0));
        Mockito.when(onLoad.getUsers()).thenReturn(mapper.readValue(userResource.getInputStream(), new TypeReference<List<User>>(){}));
