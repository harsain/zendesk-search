...
public class OrganisationSearchCommandTest {
  ...
  public void userSearchCommandTest() {
    ...
    Assertions.assertEquals(methodTarget.getMethod(), ReflectionUtils
        .findMethod(OrganisationSearchCommand.class, "search", String.class, String.class));
    Assertions.assertTrue(methodTarget.getAvailability().isAvailable());
    Assertions.assertEquals(expected,
        ReflectionUtils.invokeMethod(methodTarget.getMethod(), methodTarget.getBean(), "_id", "101")
            .toString());
  }
  ...
  public void userSearchCommandInValidTest() {
    ...
    Exception exception = Assertions.assertThrows(NoOrganisationFoundException.class,
        () -> ReflectionUtils
            .invokeMethod(methodTarget.getMethod(), methodTarget.getBean(), "id", "101"));
    Assertions
        .assertEquals("No records found for [KEY: id] & [VALUE: 101]", exception.getMessage());
  }
}
