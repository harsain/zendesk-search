...
public class ListCommandTest {
  ...
  public void listFieldsCommandTest() {
    ...
    Assertions.assertEquals(methodTarget.getHelp(),
        "Lists all the possible fields available to search on");
    Assertions.assertEquals(methodTarget.getMethod(),
        ReflectionUtils.findMethod(ListFieldsCommand.class, "list"));
    Assertions.assertTrue(methodTarget.getAvailability().isAvailable());
    Assertions.assertEquals(expectedString.trim(),
        ReflectionUtils.invokeMethod(methodTarget.getMethod(), methodTarget.getBean()).toString());
  }
}
